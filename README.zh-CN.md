...
一个轻量级的 Windows 快速启动面板，基于 Tauri (Rust + React) 构建。按 `Ctrl + Space` 显示面板，再按一次隐藏，或按 `Esc` 隐藏。
...
- **自定义背景**: 上传图片作为面板背景，支持拉伸、平铺、居中，可调节高斯模糊和透明度
...
1. 启动 ChocoBar - 系统托盘区出现图标
2. 按 `Ctrl + Space` 显示面板，再按一次隐藏，或按 `Esc` 隐藏
3. 点击 `+` 搜索并添加应用程序
4. 切换编辑模式可通过拖放排列应用
5. 在设置中可自定义快捷键、上传背景图、调节透明度和模糊
6. 右键点击托盘图标可完全退出
